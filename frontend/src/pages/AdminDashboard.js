                  ...
                  {loading ? <p>Loading...</p> : pendingOperators.length > 0 ? (
                    <ListGroup variant="flush">
                      {pendingOperators.map((op) => (
                        // MODIFIED THIS ENTIRE LISTGROUP.ITEM
                        <ListGroup.Item 
                          key={op._id} 
                          className="application-item d-flex justify-content-between align-items-center"
                        >
                          {/* Info on the left */}
                          <span> 
                            <strong>Company Name:</strong> {op.operatorDetails?.companyName || 'N/A'}
                          ...
                          <div>
                            <Button variant="success" size="sm" className="me-2" onClick={() => handleApprove(op._id)}>Approve</Button>
                            <Button variant="danger" size="sm" onClick={() => handleReject(op._id)}>Reject</Button>
                          </div>
                        </ListGroup.Item>
                      ))}
                    </ListGroup>
